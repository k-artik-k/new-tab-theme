...
- Terminal-style command launcher with compact autocomplete
- Full layout themes: terminal, neo brutalism, liquid glass, and Windows 7 aero
...
- Retro games: chicken, snake, pacman, and tetris
...
/layout edit off
/widget list
/widget toggle calendar
/config
/config accent green
/config accent #39c5bb
/config storage
/shortcut list
...
The Pacman mode is a local reimplementation, not proprietary original source code.

## Config

Layout themes are switched with `/theme`; terminal layout is locked, while alternate themes can be repositioned with `/layout edit on`.
...
layout.js        per-theme draggable and resizable widget layout
